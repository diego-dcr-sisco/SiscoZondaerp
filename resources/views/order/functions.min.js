const dayMap = {
    D: 0, // Domingo
    L: 1, // Lunes
    M: 2, // Martes
    Mi: 3, // Miércoles
    J: 4, // Jueves
    V: 5, // Viernes
    S: 6, // Sábado
};
const day_letters = ["L", "M", "Mi", "J", "V", "S", "D"];

var selected_services = [];
var aux = "";
var cost = 0;
var is_submitting_order = false;

// Genera la fecha de finalizacion del contrato
function set_endDate() {
    var startDateValue = $("#startdate").val();

    if (startDateValue) {
        var startDate = new Date(startDateValue);

        var endDate = new Date(startDate);
        endDate.setFullYear(endDate.getFullYear() + 1);
        endDate.setDate(endDate.getDate() - 1);

        var endDateFormatted = endDate.toISOString().split("T")[0];

        $("#enddate").val(endDateFormatted);
    }
}


function createServicesList() {
    showServices();
}

function showServices() {
    var html = "";
    var price = $("#price").val();
    cost = 0;
    selected_services.forEach((service, i) => {
        html += createService(service, i);
    });
    $("#cost").val(cost);
    $("#price").val(price != 0 ? price : cost * 1.5);
    $("#selected-services").html(html);
}

function deleteService(index) {
    if (index < 0 || index >= selected_services.length) {
        //console.error("Índice no válido");
        return;
    }
    if (index != -1) {
        var service = selected_services[index];
        selected_services.splice(index, 1);
        $("#service" + service.id).removeClass("active");
        selected_services.forEach((item, idx) => (item.index = idx));
    }

    showServices();
}

function generateOrder() {
    if (is_submitting_order) {
        return;
    }

    const form = document.getElementById("order-form");
    const startTime = $("#start-time").val();
    const programmedDate = $("#programmed-date").val();
    const technicians =
        $("#technicians").val() != ""
            ? JSON.parse($("#technicians").val())
            : [];
    const customer = $("#customer-id").val();

    if (startTime == "" || programmedDate == "") {
        alert("Incluye la hora de inicio y la fecha programada");
        return;
    }

    if (customer == "") {
        alert("Selecciona el cliente");
        return;
    }

    if (selected_services.length <= 0) {
        alert("Añade y/o configura los servicios");
        return;
    }

    if (technicians.length <= 0) {
        alert("Selecciona a los técnicos");
        return;
    }

    $("#services").val(
        JSON.stringify(services_configuration)
    );

    is_submitting_order = true;
    $("#order-form button[type='button']").prop("disabled", true);
    form.requestSubmit();
}

function saveDescription(index) {
    if (index != -1) {
        selected_services[index].propagate_description =
            typeof getSummaryDescribeHtml === "function"
                ? getSummaryDescribeHtml()
                : $("#summary-describe").summernote("code");
    }
    $("#describeModal").modal("hide");
}
